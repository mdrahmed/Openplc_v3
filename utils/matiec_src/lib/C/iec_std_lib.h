...
static inline TIME __time_mul(TIME IN1, LREAL IN2){
  ...
  TIME res = {(long)s + ns.quot,
		        static_cast<long> (ns.rem + (s_f - s) * 1000000000 ) };
  __normalize_timespec(&res);
  return res;
}
...
static inline TIME   __real_to_time(LREAL IN) {return (TIME){static_cast<long>(IN), static-cast<long>((IN - (LINT)IN) * 1000000000 )};}
