...
int createSocket(uint16_t port)
{
    ...
    if (socket_fd<0)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "Server: error creating stream socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    {
        sprintf(reinterpret_cast<char*>(log_msg), "Server: error binding socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    listen(socket_fd,5);
    sprintf(reinterpret_cast<char*>(log_msg), "Server: Listening on port %d\n", port);
    log(log_msg);
...
int waitForClient(int socket_fd, int protocol_type)
{
    ...
    sprintf(reinterpret_cast<char*>(log_msg), "Server: waiting for new client...\n");
    log(log_msg);
...
void *handleConnections(void *arguments)
{
    ...
    sprintf(reinterpret_cast<char*>(log_msg), "Server: Thread created for client ID: %d\n", client_fd);
    log(log_msg);
    ...
    while(*run_server)
    {
        ...
        if (messageSize <= 0 || messageSize > NET_BUFFER_SIZE)
        {
            ...
            if (messageSize == 0)
            {
                sprintf(reinterpret_cast<char*>(log_msg), "Modbus Server: client ID: %d has closed the connection\n", client_fd);
                log(log_msg);
            }
            else
            {
                sprintf(reinterpret_cast<char*>(log_msg), "Modbus Server: Something is wrong with the  client ID: %d message Size : %i\n", client_fd, messageSize);
                log(log_msg);
            }
            break;
        }
    ...
    close(client_fd);
    sprintf(reinterpret_cast<char*>(log_msg), "Terminating Modbus connections thread\r\n");
    log(log_msg);
    pthread_exit(NULL);
}
...
void startServer(uint16_t port, int protocol_type)
{
    ...
    while(*run_server)
    {
        ...
        if (client_fd < 0)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "Server: Error accepting client!\n");
            log(log_msg);
        }
        ...
        else
        {
            ...
            pthread_t thread;
            int ret = -1;
            sprintf(reinterpret_cast<char*>(log_msg), "Server: Client accepted! Creating thread for the new client ID: %d...\n", client_fd);
            log(log_msg);
    ...
    close(socket_fd);
    close(client_fd);
    sprintf(reinterpret_cast<char*>(log_msg), "Terminating Server thread\r\n");
    log(log_msg);
}
