...
{	
	...
	else if (enipType < 0)
	{
		...
		sprintf(reinterpret_cast<char*>(log_msg), "ENIP: Received unsupported EtherNet/IP Type\n");
		log(log_msg);
	}
    ...
    {
        p += sprintf(reinterpret_cast<char*>(p), "Unknown EtherNet/IP request: ");
        for (int i = 0; i < buffer_size; i++)
        {
            p += sprintf(reinterpret_cast<char*>(p), "%02x ", (unsigned char)buffer[i]);
        }
