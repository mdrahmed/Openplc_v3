...
int createSocket_interactive(int port)
{
    ...
    if (socket_fd<0)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "Interactive Server: error creating stream socket => %s\n", strerror(errno));
        log(log_msg);
        exit(1);
    }
    ...
    {
        sprintf(reinterpret_cast<char*>(log_msg), "Interactive Server: error binding socket => %s\n", strerror(errno));
        log(log_msg);
        exit(1);
    }
    // we accept max 5 pending connections
    listen(socket_fd,5);
    sprintf(reinterpret_cast<char*>(log_msg), "Interactive Server: Listening on port %d\n", port);
    log(log_msg);
...
void processCommand(unsigned char *buffer, int client_fd)
{
    ...
    {
        count_char = sprintf(reinterpret_cast<char*>(buffer), "Processing command...\n");
        write(client_fd, buffer, count_char);
        return;
    }
    
    if (strncmp(reinterpret_cast<char*>(buffer), "quit()", 6) == 0)
    {
        processing_command = true;
        sprintf(reinterpret_cast<char*>(log_msg), "Issued quit() command\n");
        log(log_msg);
        if (run_modbus)
        {
            run_modbus = 0;
            pthread_join(modbus_thread, NULL);
            sprintf(reinterpret_cast<char*>(log_msg), "Modbus server was stopped\n");
            log(log_msg);
        }
        if (run_dnp3)
        {
            ...
            sprintf(reinterpret_cast<char*>(log_msg), "DNP3 server was stopped\n");
            log(log_msg);
        }
        run_openplc = 0;
        processing_command = false;
    }
    else if (strncmp(reinterpret_cast<char*>(buffer), "start_ethercat(", 15) == 0)
    {
        processing_command = true;
        char *argument;
        ...
        free(argument);
        sprintf(reinterpret_cast<char*>(log_msg), "Issued start_ethercat() command to start with config: %s\n", ethercat_conf_file);
        log(log_msg);
        //Configure ethercat
        ethercat_configured = configureEthercat();
        processing_command = false;
    }
    else if (strncmp(reinterpret_cast<char*>(buffer), "start_modbus(", 13) == 0)
    {
        processing_command = true;
        modbus_port = readCommandArgument(buffer);
        sprintf(reinterpret_cast<char*>(log_msg), "Issued start_modbus() command to start on port: %d\n", modbus_port);
        log(log_msg);
        if (run_modbus)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "Modbus server already active. Restarting on port: %d\n", modbus_port);
            log(log_msg);
            //Stop Modbus server
            run_modbus = 0;
            pthread_join(modbus_thread, NULL);
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "stop_modbus()", 13) == 0)
    {
        processing_command = true;
        sprintf(reinterpret_cast<char*>(log_msg), "Issued stop_modbus() command\n");
        log(log_msg);
        if (run_modbus)
        {
            run_modbus = 0;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "start_dnp3(", 11) == 0)
    {
        processing_command = true;
        dnp3_port = readCommandArgument(buffer);
        sprintf(reinterpret_cast<char*>(log_msg), "Issued start_dnp3() command to start on port: %d\n", dnp3_port);
        log(log_msg);
        if (run_dnp3)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "DNP3 server already active. Restarting on port: %d\n", dnp3_port);
            log(log_msg);
            //Stop DNP3 server
            run_dnp3 = 0;
            pthread_join(dnp3_thread, NULL);
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "stop_dnp3()", 11) == 0)
    {
        processing_command = true;
        sprintf(reinterpret_cast<char*>(log_msg), "Issued stop_dnp3() command\n");
        log(log_msg);
        if (run_dnp3)
        {
            run_dnp3 = 0;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "start_enip(", 11) == 0)
    {
        processing_command = true;
        enip_port = readCommandArgument(buffer);
        sprintf(reinterpret_cast<char*>(log_msg), "Issued start_enip() command to start on port: %d\n", enip_port);
        log(log_msg);
        if (run_enip)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "EtherNet/IP server already active. Restarting on port: %d\n", enip_port);
            log(log_msg);
            //Stop Enip server
            run_enip = 0;
            pthread_join(enip_thread, NULL);
            sprintf(reinterpret_cast<char*>(log_msg), "EtherNet/IP server was stopped\n");
            log(log_msg);
        }
        //Start Enip server
        run_enip = 1;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "stop_enip()", 11) == 0)
    {
        processing_command = true;
        sprintf(reinterpret_cast<char*>(log_msg), "Issued stop_enip() command\n");
        log(log_msg);
        if (run_enip)
        {
            run_enip = 0;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "start_pstorage(", 15) == 0)
    {
        processing_command = true;
        pstorage_polling = readCommandArgument(buffer);
        sprintf(reinterpret_cast<char*>(log_msg), "Issued start_pstorage() command with polling rate of %d seconds\n", pstorage_polling);
        log(log_msg);
        if (run_pstorage)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "Persistent Storage server already active. Changing polling rate to: %d\n", pstorage_polling);
            log(log_msg);
        }
        //Start Enip server
        run_pstorage = 1;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "stop_pstorage()", 15) == 0)
    {
        processing_command = true;
        sprintf(reinterpret_cast<char*>(log_msg), "Issued stop_pstorage() command\n");
        log(log_msg);
        if (run_pstorage)
        {
            run_pstorage = 0;
            sprintf(reinterpret_cast<char*>(log_msg), "Persistent Storage thread was stopped\n");
            log(log_msg);
        }
        processing_command = false;
    }
    else if (strncmp(reinterpret_cast<char*>(buffer), "runtime_logs()", 14) == 0)
    {
        processing_command = true;
    ...
    else if (strncmp(reinterpret_cast<char*>(buffer), "exec_time()", 11) == 0)
    {
        processing_command = true;
        time(&end_time);
        count_char = sprintf(reinterpret_cast<char*>(buffer), "%llu\n", (unsigned long long)difftime(end_time, start_time));
        write(client_fd, buffer, count_char);
        processing_command = false;
        return;
    }
    ...
    {
        processing_command = true;
        count_char = sprintf(reinterpret_cast<char*>(buffer), "Error: unrecognized command\n");
        write(client_fd, buffer, count_char);
        processing_command = false;
        return;
    }
    
    count_char = sprintf(reinterpret_cast<char*>(buffer), "OK\n");
    write(client_fd, buffer, count_char);
}
...
void startInteractiveServer(int port)
{
    ...
    while(run_openplc)
    {
        ...
        if (client_fd < 0)
        {
            sprintf(reinterpret_cast<char*>(log_msg), "Interactive Server: Error accepting client!\n");
            log(log_msg);
        }
