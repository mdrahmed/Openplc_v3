...
int connect_to_tcp_server(uint8_t *ip_address, uint16_t port, int method)
{
    ...
    if (sockfd == -1)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: error creating TCP socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    servaddr.sin_addr.s_addr = inet_addr(reinterpret_cast<const char*>(ip_address));
    servaddr.sin_port = htons(port);
    
    //Connect to server
    if (connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) != 0)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: error connecting to server => %s\n", strerror(errno));
        log(log_msg);
        close(sockfd);
        return -1;
    }
    ...
    if (flags == -1)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: error reading flags from TCP socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    {
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: error setting flags for TCP socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
...
int send_tcp_message(uint8_t *msg, size_t msg_size, int socket_id)
{
    ...
    if (bytes_sent < 0)
    {
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: error sending msg to server => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
...
int receive_tcp_message(uint8_t *msg_buffer, size_t buffer_size, int socket_id)
{
    ...
    else
    {
        ...
        sprintf(reinterpret_cast<char*>(log_msg), "TCP Client: msg from server => %s\n", msg_buffer);
        log(log_msg);
    }
