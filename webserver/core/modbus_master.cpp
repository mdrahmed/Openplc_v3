...
void parseConfig()
{
        ...
        while (getline(cfgfile, line))
        {
            ...
            if (line_str[0] != '#' && strlen(line_str) > 1)
            {
                if (!strncmp(line_str, "Num_Devices", 11))
                {
                    ...
                    mb_devices = static_cast<struct MB_device*>(calloc(num_devices, sizeof(struct MB_device)));
                }
    ...
    else
    {
        ...
        sprintf(reinterpret_cast<char*>(log_msg), "Skipping configuration of Slave Devices (mbconfig.cfg file not found)\n");
        log(log_msg);
    }
...
void *querySlaveDevices(void *arg)
{
    while (run_openplc)
    {
        ...
        for (int i = 0; i < num_devices; i++)
        {
            ...
            {
                sprintf(reinterpret_cast<char*>(log_msg), "Device %s is disconnected. Attempting to reconnect...\n", mb_devices[i].dev_name);
                log(log_msg);
                if (modbus_connect(mb_devices[i].mb_ctx) == -1)
                {

                    sprintf(reinterpret_cast<char*>(log_msg), "Connection failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                    log(log_msg);
                ...
                {
                    sprintf(reinterpret_cast<char*>(log_msg), "Connected to MB device %s\n", mb_devices[i].dev_name);
                    log(log_msg);
            ...
            if (mb_devices[i].isConnected || rtu_port_connected)
            {
                ...
                if (mb_devices[i].discrete_inputs.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(reinterpret_cast<char*>(log_msg), "Modbus Read Discrete Input Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        bool_input_index += (mb_devices[i].discrete_inputs.num_regs);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].coils.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(reinterpret_cast<char*>(log_msg), "Modbus Write Coils failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].input_registers.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(reinterpret_cast<char*>(log_msg), "Modbus Read Input Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        int_input_index += (mb_devices[i].input_registers.num_regs);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].holding_read_registers.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(reinterpret_cast<char*>(log_msg), "Modbus Read Holding Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        int_input_index += (mb_devices[i].holding_read_registers.num_regs);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].holding_registers.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(reinterpret_cast<char*>(log_msg), "Modbus Write Holding Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
    ...
    for (int i = 0; i < num_devices; i++)
    {
        ...
        else if (mb_devices[i].protocol == MB_RTU)
        {
            ...
            if (share_index != -1)
            {
                ...
                {
                    unsigned char log_msg[1000];
                    sprintf(reinterpret_cast<char*>(log_msg), "Warning MB device %s port setting missmatch\n", mb_devices[i].dev_name);
                    log(log_msg);
                }
